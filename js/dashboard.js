...
window.onload = () => {
    ...
    this.myFunction = function (name) {
        console.log(name);
    }

    data.forEach(x => {
        let myDd = JSON.stringify(x);
        content.innerHTML += `<div onclick="myFunction( '${myDd}' )" class="card">
            <div class="card_header">
                <div class="flex-center normal-padding">
                    <img class="profile_card" src=${x.profile_image} width="60" height="60" alt="profile"/>
                    <div class="padding-left">
                        <p>${x.name}</p>
                        <p>${x.date}</p>
                    </div>
                </div>
                <object class="padding-right" data="../icons/icons/instagram-logo.svg" > </object>
           </div>
           <div>
                <img class="card_image" src=${x.image} alt='card-image' width="100%" height="360px"/>
           </div>
           ...
           <div class="card_footer">
                <span class="normal-padding">
                    <object class="padding-right" data="../icons/icons/heart.svg"> </object>
                    ${x.likes}
                </span>
            </div>
        </div>`;
        ...
        modals.forEach(function (trigger) {
            trigger.addEventListener('click', function (event, data) {
                event.preventDefault();
                let modal = document.getElementById(trigger.dataset.modal);
                modal.classList.add('open');
                modal.innerHTML = `<div> ${data.name} </div>`
        ...
        // content.appendChild(createCard(x));
    });


    // Another approach for creating content inside the body of the HTML.
    ...
    //     cardHeader.appendChild(createImage(data.image, "profile", true))
    //     cardHeader.appendChild(createParagraph(data.name))
    //
    ...
    // }
    //
    // const createImage = (image, alt, isProfile) => {
    //     const img = document.createElement('IMAGE');
    //     img.setAttribute("src", image);
    //     img.setAttribute("alt", alt);
    //     img.setAttribute("width", "304");
    //     img.setAttribute("height", "228");
    //     img.className = isProfile ? "profile_card" : "card_image";
    //
    //     return img;
    // }
    //
